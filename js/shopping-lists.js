...
// Filter/sort state for lists table
let listSearchTerm = '';
let listFilter = '';
let listSortColumn = 'name';
let listSortDirection = 'asc';
...
function initializeShoppingLists() {
    ...
    loadShoppingLists();
    console.log('Loaded shopping lists:', shoppingLists.length);
    setupEventListeners();
    setupListsTableListeners();
    renderListsTable();
}
...
function setupListsTableListeners() {
    if (listSearchInput) {
        listSearchInput.addEventListener('input', (e) => {
            listSearchTerm = (e.target.value || '').toLowerCase();
            renderListsTable();
        });
    }
    if (listStatusFilterEl) {
        listStatusFilterEl.addEventListener('change', (e) => {
    ...
    arr.sort((a, b) => {
        ...
        switch (listSortColumn) {
            ...
            case 'items': {
                va = (a.items || []).length;
                vb = (b.items || []).length;
                return listSortDirection === 'asc' ? va - vb : vb - va;
            }
            case 'created': {
...
function renderListsTable() {
    ...
    sorted.forEach(list => {
        ...
        const descDisplay = desc ? (desc.length > 60 ? desc.slice(0, 57) + '…' : desc) : '—';
        const row = document.createElement('tr');
        row.innerHTML = `
            <td><span class="ingredient-name-text">${escapeHtml(list.name || '')}</span></td>
